...
def test_like_on_post(authorized_client, test_posts):
    res = authorized_client.post("/like/", json={"post_id": test_posts[3].id, "dir": 1})
...
def test_delete_like(authorized_client, test_posts, test_like):
    res = authorized_client.post("/like/", json={"post_id": test_posts[3].id, "dir": 0})
    assert res.status_code == 201
...
def test_delete_like_on_post_not_exist(authorized_client, test_posts):
    res = authorized_client.post("/like/", json={"post_id": 10, "dir": 1})
    assert res.status_code == 404
